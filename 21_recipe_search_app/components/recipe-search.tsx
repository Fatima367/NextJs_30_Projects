...
export default function RecipeSearchComponent() {
  ...
  return (
    <div className="flex flex-col h-full w-full max-w-6xl mx-auto p-4 md:p-6 bg-gradient-to-tl
    from-yellow-50 via-white to-yellow-50">
      <header className="flex flex-col items-center mb-6">
        <h1 className="text-3xl font-bold mb-2 font-serif text-orange-800">Recipe Search</h1>
        ...
        <div className="mb-4">
          ...
          <div className="flex space-x-2">
            {examples.map((example) => (
              <span
                key={example}
                className="px-2 py-1 bg-orange-200 rounded-md cursor-pointer"
        ...
        <form className="relative w-full max-w-md mb-6" onSubmit={handleSearch}>
          <Input
            ...
            className="pr-10 ring-2 ring-orange-300 shadow-md"
            value={query}
      ...
      {loading ? (
        <div className="flex flex-col justify-center items-center w-full h-full text-orange-300">
          <Spinner className="w-10 h-10 mb-4 text-orange-300" />
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          ...
          {recipes.map((recipe) => (
            <Card className="group relative shadow-lg gap-4 bg-amber-50
            ring-2 ring-yellow-100" key={recipe.uri}>
              <Image
                src={recipe.image}
                alt={recipe.label}
                width={400}
              ...
              <CardContent className="p-4">
                <h2 className="text-xl font-bold mb-2 font-serif text-orange-900">{recipe.label}</h2>
